...
今回の機能は巨大ファイルの一部のみを読み込むことでメモリ消費を抑えるものでしたが、
その結果，巨大ファイルの全体を閲覧することができない，書き込みをおこなうと，そのbufferに読み込まれた部分のみが変更され，その他の部分は失われる，といった問題が発生しました。
upstreamにたいしてcommitを行うのであれば，巨大ファイルの読み込みを部分的に行い，スクロールに応じて必要な部分を読み込む，window方式の実装を行うことが望ましいと考えています。
この形であれば，巨大ファイルの全体を閲覧することができ，書き込みも全体の整合性を保ったまま行うことが可能になると考えられます。
